...
#define CKG_INCLUDE_TYPES
    ...
    */
    ...
    #define ckg_hashmap_init_with_hash(map, KeyType, ValueType, __key_is_ptr, __hash_function, __eq_function) \
        ...
        #define ckg_hashmap_get_pointer(map, key)      \
        (                                              \
            (map)->temp_key = (key),                   \
            ckg_hashmap_get_pointer_helper((u8*)(map)) \
        )                                              \
        ...
        #define ckg_hashmap_iterator(map, KeyType, ValueType, codeblock)  \
        for (u64 i = 0; i < map->meta.capacity; i++) {                    \
            if (!map->entries[i].filled || map->entries[i].dead) {        \
                continue;                                                 \
            }                                                             \
                                                                          \
            KeyType key = map->entries[i].key;                            \
            ValueType value = map->entries[i].value;                      \
            codeblock                                                     \
        }                                                                 \

        #define ckg_hashmap_iterator_pointer(map, KeyType, ValueType, codeblock) \
        for (int i = 0; i < map->meta.capacity; i++) {                           \
            if (!map->entries[i].filled || map->entries[i].dead) {               \
                continue;                                                        \
            }                                                                    \
                                                                                 \
