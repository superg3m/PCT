...
#define CORE_FRONTEND
    ...
    */
    ...
    template<typename K, typename V>
    V* hashmap_get_pointer(Hashmap<K, V>* h, K key) {
        RUNTIME_ASSERT_MSG(hashmap_has(h, key), "Key doesn't exist\n");
    ...
    #define X_KEYWORD_TOKENS               \
        ...
        X(TS_KEYWORD_DEFER, "defer")       \
        X(TS_KEYWORD_CONTINUE, "continue") \
        X(TS_KEYWORD_BREAK, "break")       \

    #define X_PRIMITIVE_TYPES_TOKENS     \
        X(TS_PRIMITIVE_VOID, "void")     \
                                         \
        X(TS_PRIMITIVE_U8,  "u8")        \
        X(TS_PRIMITIVE_U16, "u16")       \
        X(TS_PRIMITIVE_U32, "u32")       \
        X(TS_PRIMITIVE_U64, "u64")       \
                                         \
        X(TS_PRIMITIVE_S8,  "s8")        \
        X(TS_PRIMITIVE_S16, "s16")       \
        X(TS_PRIMITIVE_S32, "s32")       \
        X(TS_PRIMITIVE_S64, "s64")       \
                                         \
        X(TS_PRIMITIVE_F32, "f32")       \
        X(TS_PRIMITIVE_F64, "f64")       \
                                         \
...
//
...
#if defined(CORE_FRONTEND_IMPL)
    void Token::print() const {
        LOG_TRACE("%s(%.*s) | line: %d\n", token_to_string(this->type), (int)this->sv.length, this->sv.data, this->line);
    }
    ...
    const char* token_to_string(TokenType type) {
        ...
        STATIC_ASSERT(ARRARY_COUNT(token_strings) == TOKEN_COUNT);

        return token_strings[type];
    }

    const char* token_to_string(Token token) {
        return token_to_string(token.type);
    }

    Token token_invalid() {
        Token t = {};
        t.type = TOKEN_ILLEGAL_TOKEN;
        t.line = 404;
    ...
    Token token_keyword_from_source_view(View<char> sv, int line) {
        static Hashmap<View<char>, TokenType> keyword_map = hashmap_create<View<char>, TokenType>(allocator_general(), {
            #define X(name, str) KeyValuePair<View<char>, TokenType>{ view_create<char>(str, sizeof(str) - 1), name },
                X_KEYWORD_TOKENS
            #undef X
        });

        Token ret = token_invalid(); // Invalid
        ret.sv = sv;
        ret.line = line;
    ...
    Token token_syntax_from_source_view(View<char> sv, int line) {
        static Hashmap<View<char>, TokenType> syntax_map = hashmap_create<View<char>, TokenType>(allocator_general(), {
            #define X(name, str) KeyValuePair<View<char>, TokenType>{ view_create<char>(str, sizeof(str) - 1), name },
                X_SYNTAX_TOKENS
            #undef X
        });
        ...
        if (hashmap_has(&syntax_map, sv)) {
            ret.type = hashmap_get(&syntax_map, sv);
        }
    ...
    Token token_literal_from_source_view(View<char> sv, int line) {
        Token ret = token_invalid();
        ret.sv = sv;
        ret.line = line;

        if (ret.sv.data[0] == '"') {
            ret.type = TS_LITERAL_STRING;
    ...
    Token token_primitive_from_source_view(View<char> sv, int line) {
        static Hashmap<View<char>, TokenType> primitive_type_map = hashmap_create<View<char>, TokenType>(allocator_general(), {
            #define X(name, str) KeyValuePair<View<char>, TokenType>{ view_create<char>(str, sizeof(str) - 1), name },
                X_PRIMITIVE_TYPES_TOKENS
            #undef X
        });
        ...
        if (hashmap_has(&primitive_type_map, sv)) {
            ret.type = hashmap_get(&primitive_type_map, sv);
        }
    ...
    INTERNAL_FUNCTION Lexer create_lexer(View<char> source, Vector<Token>* out_tokens) {
        ...
        ret.c = '\0';
        ret.source = source;
        ret.tokens = out_tokens;

        return ret;
    };

    INTERNAL_FUNCTION void lexer_consume_next_character(Lexer* lexer) {
    ...
    INTERNAL_FUNCTION bool lexer_consume_on_match(Lexer* lexer, char expected) {
        if (lexer->peek_nth_character() != expected) {
            return false;
        }

        lexer_consume_next_character(lexer);
        return true;
    }

    INTERNAL_FUNCTION bool lexer_try_consume_word(Lexer* lexer) {
        if (!CHAR_IS_ALPHA(lexer->c)) {
            return false;
        }

        while (CHAR_IS_ALPHA_NUMERIC(lexer->peek_nth_character()) || lexer->peek_nth_character() == '_') {
            if (lexer->is_eof()) {
                break;
            }
    ...
    INTERNAL_FUNCTION bool lexer_consume_word(Lexer* lexer) {
        if (!lexer_try_consume_word(lexer)) {
            return false;
        }

        View<char> sv = lexer->scratch_buffer();

        Token token = token_primitive_from_source_view(sv, lexer->line);
        ...
        token = token_keyword_from_source_view(sv, lexer->line);
        if (token.type != TOKEN_ILLEGAL_TOKEN) {
            vector_append(lexer->tokens, token);
    ...
    INTERNAL_FUNCTION void lexer_consume_until_new_line(Lexer* lexer) {
        while (!lexer->is_eof() && lexer->peek_nth_character() != '\n') {
            lexer_consume_next_character(lexer);
        }
    }

    INTERNAL_FUNCTION bool lexer_consume_syntax(Lexer* lexer) {
        if (lexer->c == '/') {
            if (lexer_consume_on_match(lexer, '/')) {
                ...
                while (!(lexer->peek_nth_character() == '*' && lexer->peek_nth_character(1) == '/')) {
                    if (lexer->is_eof()) {
                        lexer->report_error("Multiline comment doesn't terminate\n");
                    }
                    
                    lexer_consume_next_character(lexer);
                }

                lexer_consume_next_character(lexer); // consume '*'
                lexer_consume_next_character(lexer); // consume '/'
        ...
        View<char> sv = lexer->scratch_buffer();
        Token token = token_syntax_from_source_view(sv, lexer->line);
        if (token.type != TOKEN_ILLEGAL_TOKEN) {
            vector_append(lexer->tokens, token);
            return true;
        }
    ...
    void lexer_consume_digit_literal(Lexer* lexer) {
        ...
        while (CHAR_IS_DIGIT(lexer->peek_nth_character()) || lexer->peek_nth_character() == '.') {
            lexer_consume_next_character(lexer);
        }

        View<char> sv = lexer->scratch_buffer();
        Token token = token_literal_from_source_view(sv, lexer->line);
        vector_append(lexer->tokens, token);
    }

    INTERNAL_FUNCTION void lexer_consume_string_literal(Lexer* lexer) {
        while (lexer->peek_nth_character() != '\"') {
            if (lexer->is_eof()) {
                lexer->report_error("String literal doesn't have a closing double quote!\n");
            }
        ...
        Token token = token_create(TS_LITERAL_STRING, sv, lexer->line);
        vector_append(lexer->tokens, token);
    }

    void lexer_consume_character_literal(Lexer* lexer) {
        if (lexer_consume_on_match(lexer, '\'')) {
            lexer->report_error("character literal doesn't have any ascii data in between\n");
        }
    ...
    bool lexer_consume_literal(Lexer* lexer) {
        bool has_unary = (lexer->c == '-' || lexer->c == '+' );
        if (CHAR_IS_DIGIT(lexer->c) || (has_unary && CHAR_IS_DIGIT(lexer->peek_nth_character()))) {
    ...
    void lexer_consume_next_token(Lexer* lexer) {
        ...
        if (is_whitespace(lexer->c)) {}
            ...
            lexer->report_error("Illegal token found\n");
        }
    }

    char Lexer::peek_nth_character(u64 n) const {
    ...
    void Lexer::report_error(const char* msg) const {
        View<char> scratch = this->scratch_buffer();
    ...
    View<char> Lexer::scratch_buffer() const {
        return view_create<char>((char*)this->source.data + this->left_pos, this->right_pos - this->left_pos);
    }

    void lexer_generate_tokens(u8* data, size_t file_size, Vector<Token>* out_tokens) {
        View<char> source = view_create<char>((char*)data, file_size);
        Lexer lexer = create_lexer(source, out_tokens);
        while (!lexer.is_eof()) {
    ...
    Parser parser_create(Allocator allocator, Vector<Token>* in_tokens) {
        ...
        parser.tokens = in_tokens;
        parser.current = 0;

        return parser;
    }

    Token Parser::peek_nth_token(int n) const {
        if (this->current + n >= this->tokens->count) {
            return token_invalid(); // invalid
        }
    ...
    void Parser::report_error(const char* fmt, ...) const {
        Token token = this->peek_nth_token();

        va_list args;
        va_start(args, fmt);
        LOG_ERROR("String: %s\n", token_to_string(token));
        LOG_ERROR("Error Line: %d | %s\n", token.line, str_sprintf(this->allocator, nullptr, fmt, args));
        va_end(args);
    ...
    // TODO(Jovanni): This is actually really fuckign misleading because if
    // you expect some token type and you don't get it you report the error but have
    // no real way to know if you failed this check
    Token parser_expect(Parser* parser, TokenType expected_type) {
        Token got_token = parser->peek_nth_token();

        if (got_token.type != expected_type) {
            const char* expected_str = token_to_string(expected_type);
            const char* got_str = token_to_string(got_token);
            parser->report_error("Expected: %s | Got: %s\n", expected_str, got_str);
        }

        return parser_consume_next_token(parser);
    }

    bool parser_consume_on_match(Parser* parser, TokenType expected_type) {
        if (parser->peek_nth_token().type == expected_type) {
    ...
    static const char* indentFromDepth( int depth, const char* indent) {
        if (depth == 0) {
            return "";
        }

        size_t allocation_size = str_length(indent) * depth;
        char* ret = (char*)std::malloc(allocation_size + 1);
    ...
    static const char* toStringHelper(JSON* root, const char* indent, int depth) {
        switch (root->type) {
            case JSON_VALUE_BOOL: {
                return str_sprintf(root->allocator, nullptr, "%s", root->boolean ? "true" : "false");
            } break;

            case JSON_VALUE_INT: {
                return str_sprintf(root->allocator, nullptr, "%d", root->integer);
            } break;

            case JSON_VALUE_FLOAT: {
                return str_sprintf(root->allocator, nullptr, "%f", root->floating);
            } break;

            case JSON_VALUE_STRING: {
                return str_sprintf(root->allocator, nullptr, "\"%.*s\"", root->string.length, root->string.data);
            } break;
            ...
            case JSON_VALUE_ARRAY: {
                u64 array_count = root->array.elements.count;
                Vector<u64> lengths = vector_create<u64>(root->allocator, array_count);
                const char** buffers = (const char**)std::malloc(sizeof(char*) * array_count);

                size_t total_allocation_size = 1; // 1 for the null terminator
                const char* member_indent = indentFromDepth(depth + 1, indent);
                for (int i = 0; i < array_count; i++) {
                    u64 temp_length = 0;
                    JSON* element = root->array.elements[i];
                    const char* value = toStringHelper(element, indent, depth + 1);
                    if (i == array_count - 1) {
                        buffers[i] = str_sprintf(root->allocator, &temp_length, "%s%s", member_indent, value);
                    } else {
                        buffers[i] = str_sprintf(root->allocator, &temp_length, "%s%s,\n", member_indent, value);
                    }

                    vector_append(&lengths, temp_length);
                    total_allocation_size += temp_length;
                }
                
                u64 string_length = 0;
                char* buffer = (char*)std::malloc(total_allocation_size);
                for (int i = 0; i < array_count; i++) {
                    str_append(buffer, string_length, total_allocation_size, buffers[i], lengths[i]);
                }

                return str_sprintf(root->allocator, nullptr, "[\n%s\n%s]", buffer, indentFromDepth(depth, indent));
            } break;

            case JSON_VALUE_OBJECT: {
                int object_member_count = root->object.pairs.count;
                Vector<u64> lengths = vector_create<u64>(root->allocator, object_member_count);
                const char** buffers = (const char**)std::malloc(sizeof(char*) * object_member_count);

                size_t total_allocation_size = 1; // 1 for the null terminator
                const char* member_indent = indentFromDepth(depth + 1, indent);
                for (int i = 0; i < object_member_count; i++) {
                    KeyValuePair pair = root->object.pairs[i];
                    u64 temp_length = 0;

                    const char* value = toStringHelper(pair.value, indent, depth + 1);
                    if (i == object_member_count - 1) {
                        buffers[i] = str_sprintf(root->allocator, &temp_length, "%s\"%s\": %s", member_indent, pair.key, value);
                    } else {
                        buffers[i] = str_sprintf(root->allocator, &temp_length, "%s\"%s\": %s,\n", member_indent, pair.key, value);
                    }
                ...
                u64 temp = 0;
                return str_sprintf(root->allocator, &temp, "{\n%s\n%s}", buffer, indentFromDepth(depth, indent));
            } break;
        }
    ...
    const char* JSON::ToString(JSON* root, const char* indent) {
        if (root == nullptr) {
            return "JSON* root = null";
        }

        return toStringHelper(root, indent, 0);
    }


    static JSON* parseHelper(JSON* root, Parser* parser) {
        RUNTIME_ASSERT(root);

        Token token = parser_consume_next_token(parser);

        switch (token.type) {
            case TS_KEYWORD_FALSE:
            case TS_KEYWORD_TRUE: {
                return JSON::Boolean(root->allocator, token.b);
            } break;

            case TS_LITERAL_INTEGER: {
                return JSON::Integer(root->allocator, token.i);
            } break;

            case TS_LITERAL_FLOAT: {
                return JSON::Floating(root->allocator, token.f);
            } break;

            case TS_LITERAL_STRING: {
                return JSON::String(root->allocator, token.sv);
            } break;

            case TS_KEYWORD_NULL: {
                return JSON::Null(root->allocator);
            } break;

            case TS_LEFT_BRACKET: {
                JSON* ret = JSON::Array(root->allocator);
                while (parser->peek_nth_token().type != TOKEN_ILLEGAL_TOKEN && !parser_consume_on_match(parser, TS_RIGHT_BRACKET)) {
                    if (parser->peek_nth_token().type == TOKEN_ILLEGAL_TOKEN) {
                        return nullptr;
                    }
                    
                    JSON* value = parseHelper(ret, parser);
                    if (value == nullptr) {
                        return nullptr;
                    }

                    vector_append(&ret->array.elements, value);
                    parser_consume_on_match(parser, TS_COMMA);
                }
            ...
            case TS_LEFT_CURLY: {
                ...
                while (parser->peek_nth_token().type != TOKEN_ILLEGAL_TOKEN && !parser_consume_on_match(parser, TS_RIGHT_CURLY)) {
                    if (parser->peek_nth_token().type == TOKEN_ILLEGAL_TOKEN) {
                        return nullptr;
                    }

                    Token key_token = parser_consume_next_token(parser);
                    if (key_token.type != TS_LITERAL_STRING) {
                        LOG_ERROR("JSON Parser Expected: %s | got %s\n", token_to_string(TS_LITERAL_STRING), token_to_string(key_token));
                        return nullptr;
                    }
    ...
    JSON* JSON::Parse(Allocator allocator, const char* json_string, u64 json_string_length) {
        ...
        return parseHelper(root, &parser);
    }

    JSON* JSON::Object(Allocator allocator) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
        ret->object.keys = hashmap_create<const char*, u64>(allocator);
        ret->object.pairs = vector_create<KeyValuePair<const char *, JSON *>>(allocator);
    ...
    JSON* JSON::Array(Allocator allocator) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
        ret->array.elements = vector_create<JSON*>(allocator);
    ...
    JSON* JSON::Null(Allocator allocator) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
    ...
    JSON* JSON::Integer(Allocator allocator, int value) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
        ret->allocator = allocator;
    ...
    JSON* JSON::Floating(Allocator allocator, float value) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
        ret->allocator = allocator;
    ...
    JSON* JSON::Boolean(Allocator allocator, bool value) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
    ...
    JSON* JSON::String(Allocator allocator, View<char> value) {
        JSON* ret = (JSON*)std::malloc(sizeof(JSON));
    ...
    JSON* JSON::String(Allocator allocator, char* str) {
        u64 length = str_length(str);
        return JSON::String(allocator, view_create<char>(str, length));
    }

    JSON* JSON::String(Allocator allocator, const char* str) {
        u64 length = str_length(str);
        return JSON::String(allocator, view_create<char>(str, length));
    }
#endif